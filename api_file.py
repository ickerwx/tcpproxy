...
class API():
    ...
    def __init__(self, args, infos, uri):
        ...
        filename, extension = os.path.splitext(self.uri.path)

        if extension in ["json"]:
            with open(self.uri.path, 'r') as f:
                for line in f.readlines():
                    line = self.lines.append(linesplit('#')[0].strip())
                    lines.append(json.loads(line))

        elif extension in ['csv']:
            with open(self.uri.path, 'r') as f:
                self.lines = csv.reader(self.__decomment(f), skipinitialspace=True, quoting=csv.QUOTE_MINIMAL)
                self.lines = list(lines)

        if not lines:
            logger.warning('%s empty (or only invalid/comments lines)' % iru.path)
